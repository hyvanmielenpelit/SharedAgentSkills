...
> `skills/`, `skills-claude/`, `skills-gemini/`, `skills-codex/`, and `rules/`.
> Moving a file out of one of those into here silently uninstalls it from every
> machine; adding a maintenance note to one of those turns it into a
> globally-triggering skill in every project.
...
- **You are editing the globally installed skills.** `~/.claude/skills/*` and
  `$CODEX_HOME/skills/*` are junctions into this working tree, while
  `~/.gemini/config/skills.json` points to its source directories. An edit to a
  file inside an existing skill is therefore live for its routed harnesses on
  this machine immediately — before any commit. What is *not* live: new or
  renamed skill directories for Claude or Codex (need `setup.ps1`),
  `rules/AGENTS.md` / `rules/GEMINI.md` for Antigravity, and
  `rules/AGENTS.md` / `rules/CODEX.md` for Codex. The latter two harnesses
  receive inlined copies.
