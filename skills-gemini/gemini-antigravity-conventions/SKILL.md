...
`claude-code-conventions`; the Codex equivalents are in `codex-conventions`.
...
This is the common case: plans are frequently written in another application and
implemented here.
...
> to another application's models, and no configuration makes it so.
...
document in the plans repository is the entire interface.

If asked to hand work to another application, say plainly that this one cannot, and point
...
- Skills whose names begin with `claude-` or `codex-` are deliberately **not** installed here. They describe mechanics this application does not have.
...
- **`.claude/`** is the Claude Code adapter layer. Its `skills/` entries are
...
> silently out of date. Claude Code reads the shared rules through a live link. Codex, like
> Antigravity, receives an inlined copy, so either copy can lag behind the sources.
